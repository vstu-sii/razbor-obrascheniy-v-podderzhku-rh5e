...
Числа ниже — не опубликованные характеристики моделей и не средние значения датасета, а внутренние верхние лимиты команды для проектирования одного запроса. Основания и воспроизводимый расчёт зафиксированы в [ноутбуке эксперимента](../notebooks/lab1_prompt_experiments.ipynb).

| Компонент | Лимит | Основание оценки |
| --- | ---: | --- |
| Системная инструкция и JSON Schema | 450 токенов | В smoke-тесте одна инструкция заняла 214–283 входных токена по `api_usage`; до 450 оставлен резерв на схему и служебную обвязку: [исходные замеры и расчёт](../notebooks/lab1_prompt_experiments.ipynb). |
| Одно обращение | 250 токенов | Проектный лимит команды на текст одного входящего тикета до сокращения: [параметры бюджета](../notebooks/lab1_prompt_experiments.ipynb). |
| История | 600 токенов | Проектный лимит после отбора релевантных сообщений и обрезки длинной истории: [параметры бюджета](../notebooks/lab1_prompt_experiments.ipynb). |
| Найденные фрагменты БЗ | 900 токенов | Не более трёх retrieval-фрагментов примерно по 300 токенов: [формула бюджета](../notebooks/lab1_prompt_experiments.ipynb). |
| Структурированный результат | 500 токенов | Целевой лимит видимого JSON-ответа; GPT в smoke-тесте использовал 252 completion-токена: [исходный замер и резерв](../notebooks/lab1_prompt_experiments.ipynb). |

Итого проектный лимит одного вызова: `450 + 250 + 600 + 900 + 500 = 2700` токенов. Один разрешённый полный повтор даёт верхнюю оценку `2700 × 2 = 5400` токенов: [воспроизводимый расчёт](../notebooks/lab1_prompt_experiments.ipynb).
